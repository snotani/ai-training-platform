...
export async function getProfileSummary(): Promise<ProfileSummary | null> {
  ...
  const [profileRes, streakRes, badgesRes, certsRes] = await Promise.all([
    supabase.from("profiles").select("xp, display_name, region").eq("id", user.id).maybeSingle(),
...
export async function updateMyRegion(region: string | null): Promise<{ ok: boolean }> {
  const supabase = await createClient();
